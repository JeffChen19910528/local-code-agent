...
**`web_fetch` 讀不到用 JavaScript 動態載入內容的網站：** `web_fetch` 只是單純的 HTTP GET 再把 HTML 標籤剝掉，不會執行 JavaScript。像中央氣象署這類「畫面內容要靠前端 JS 抓 API 才會出現」的網站，抓回來的只會是導覽列、選單這些結構性文字，沒有實際的天氣數字——模型通常會誠實地說明抓不到內容，而不是編造答案，這部分行為是對的。真正的解法是換一個不需要 JS 也能讀到內容的來源：針對天氣查詢，system prompt 現在會提示模型改用 `https://wttr.in/<城市>?format=j1`（含未來幾天預報的 JSON）或 `?format=4`（單行摘要）這個純文字氣象服務，也可能像上面範例一樣改抓 AccuWeather 等有把預報資料直接渲染進 HTML 的網站。
