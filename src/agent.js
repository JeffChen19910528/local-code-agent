...
export function buildSystemPrompt(config, tools) {
  return [
    ...
    "web_fetch only reads a page's raw HTML - it cannot run JavaScript, so pages that render their real content client-side (common on official/government sites and many weather sites) will come back as mostly navigation/boilerplate with no actual data. If that happens, do not just give up or send the user away - retry with a source that returns plain text/JSON directly instead. In particular, for a weather question, web_fetch `https://wttr.in/<city>?format=j1` for a detailed JSON forecast (current conditions plus a few days ahead) or `https://wttr.in/<city>?format=4` for a quick one-line summary - both are plain text, need no JavaScript, and work with a plain fetch.",
